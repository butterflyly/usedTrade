...
import com.example.usedTrade.Security.CustomUserDetails;
...
public class JwtProvider {
    ...
    // 유저 정보를 가지고 AccessToken, RefreshToken을 생성하기
    public JwtToken generateToken(Authentication authentication) {
        ...
        // UserRole 가져오기
        String roleCode = "N"; // 기본 NORMAL
        if (principalObj instanceof CustomUserDetails custom) {
            UserRole userRole = custom.getRole();
            roleCode = userRole.getCode(); // DB 저장용 코드: "B", "N", "A"
        }
        ...
        // AccessToken 생성
        Date accessTokenExpire = new Date(now + ACCESS_TOKEN_EXPIRE_TIME); // 엑세스 토큰 기간 설정
        String accessToken = generateAccessToken(username, authorities, accessTokenExpire,roleCode);
    ...
    private String generateAccessToken(String username, String authorities, Date expireDate,String roleCode) {
        return Jwts. // JWT(토큰)를 생성하고 검증할 때 사용하는 유틸리티 클래스를 제공하는 라이브러리의 핵심 진입점
                builder()
                .setSubject(username) // 토큰 제목 (사용자 이름)
                .claim("auth", authorities) // 권한 정보 (커스텀 클레임)
                .claim("role", roleCode)    // DB UserRole 코드
    ...
    public String generateAccessTokenOnly(Authentication authentication) {
        ...
        if (principal instanceof CustomUserDetails custom) {
            roleCode = custom.getRole().getCode();
        }
    ...
    public boolean isTokenExpired(String token) {
        try {
            Jwts.parserBuilder()
                    .setSigningKey(key)
                    // 약간의 clock skew 허용 (초 단위). 서버 시간 미세 차이로 인한 문제 방지.
    ...
    public Authentication getAuthentication(String accessToken) {
        ...
        Collection<? extends GrantedAuthority> authorities =
                Arrays.stream(claims.get("auth").toString(). // 사용자의 권한정보 문자열 변환
                                split(",")) // 쉼표로 인해 문자열 배열 생성
               // SimpleGrantedAuthority는 스프링 시큐리티에서 권한(ROLE) 객체
